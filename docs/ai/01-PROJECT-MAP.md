...
## Offline Runtime Asset Rule

ERPSMART must not depend on `unpkg.com` or other CDN resources for core runtime JavaScript. Vue global runtime is served from `public/vendor/vue` and generated by `scripts/sync-local-vue-assets.mjs`. See `docs/ai/03-architecture/offline-frontend-assets.md`.

## 2026-06-22 Warehouse Notes capability

Warehouse has entered the ecosystem-integration phase. The module is no longer only CRUD/import/export; it now begins attaching shared platform capabilities.

New Notes integration files:
...
Builder implication: reusable capabilities such as Notes must be represented as capability templates that generate backend relationships, frontend detail tabs, validation checklists, and RAG manifest entries.
